    ...
    <div class="container-fluid">
        ...
        <div class="stories-header d-flex justify-content-between align-items-center flex-wrap">
            ...
            <div class="stories-title">
                <h3 class="mb-0">Modifier une success story</h3>
                <span class="stories-subtitle">Édition d'une success story existante</span>
            </div>
        ...
        <div class="container-fluid">
            <div class="card card-warning">
                <div class="card-header">
                    <h3 class="card-title">Edition de la success story</h3>
                </div>
                ...
                <form action="{{ route('success-stories.update', $successStory) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="titre" id="titre" class="form-control" value="{{ old('titre', $successStory->titre) }}" required>
                        </div>

                        <div class="mb-3">
                            <label for="description" class="form-label">Description</label>
                            <div class="editor-shell">
                                <textarea name="description" id="description" rows="10" class="form-control" required>{{ old('description', $successStory->description) }}</textarea>
                            </div>
                        </div>
                            ...
                            <div class="row g-3">
                                @forelse ($successStory->images as $image)
                                    @php
                                        ...
                                        $selectedCoverId = (int) old('cover_image_id', $successStory->imageId);
                                    @endphp
                                    <div class="col-sm-6 col-md-4 col-lg-3">
                                        <div class="existing-image-card h-100">
                                            <img src="{{ asset('storage/' . $image->image_path) }}" alt="Image success story {{ $image->id }}">
                                            <div class="p-3">
                                ...
                                @empty
                                    <div class="col-12">
                                        <div class="alert alert-secondary mb-0">Aucune image enregistree pour cette success story.</div>
                                    </div>
                                @endforelse
                            </div>
                        </div>
                    ...
                    <div class="card-footer d-flex justify-content-between">
                        <a href="{{ route('success-stories.index') }}" class="btn btn-secondary">Retour</a>
...
@push('scripts')
    <script>
        const initEditSuccessStoryPage = function () {
            const form = document.querySelector('form[action="{{ route('success-stories.update', $successStory) }}"]');
                    ...
                    reader.onload = function (event) {
                        ...
                        col.innerHTML = `
                            <div class="card preview-card h-100 position-relative">
                                <button type="button" class="preview-remove" data-index="${index}" title="Retirer cette image">
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initEditSuccessStoryPage);
        } else {
            initEditSuccessStoryPage();
        }
    </script>
@endpush

    ...
    <div class="container-fluid">
        ...
        <div class="galleries-header d-flex justify-content-between align-items-center flex-wrap">
            ...
            <div class="galleries-title">
                <h3 class="mb-0">Modifier une galerie</h3>
                <span class="galleries-subtitle">Édition d'une galerie existante</span>
            </div>
        ...
        <div class="container-fluid">
            <div class="card card-warning">
                <div class="card-header">
                    <h3 class="card-title">Edition de la galerie</h3>
                </div>
                ...
                <form action="{{ route('galleries.update', $gallery) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="titre" id="titre" class="form-control" value="{{ old('titre', $gallery->titre) }}" required>
                        </div>

                        <div class="mb-4">
                            <label class="form-label d-block">Medias existants</label>
                            <div class="row g-3">
                                @forelse ($gallery->medias as $media)
                                    @php
                                        ...
                                        $oldRemovedMedias = collect(old('remove_medias', []))->map(fn ($id) => (int) $id)->all();
                                    @endphp
                                    <div class="col-sm-6 col-md-4 col-lg-3">
                                        <div class="card existing-media-card h-100">
                                            @if ($media->media_type === 'video')
                                                <video class="existing-media" controls muted playsinline>
                                                    <source src="{{ $mediaUrl }}">
                                                </video>
                                            @else
                                                <img src="{{ $mediaUrl }}" alt="Media galerie {{ $media->id }}" class="existing-media">
                                            @endif
                                            <div class="card-body p-3">
                                                ...
                                                <div class="form-check mt-3">
                                                    <input class="form-check-input" type="checkbox" name="remove_medias[]" value="{{ $media->id }}" id="remove_media_{{ $media->id }}" {{ in_array($media->id, $oldRemovedMedias, true) ? 'checked' : '' }}>
                                ...
                                @empty
                                    <div class="col-12">
                                        <div class="alert alert-secondary mb-0">Aucun media enregistre pour cette galerie.</div>
                                    </div>
                                @endforelse
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            <label class="form-label d-block">Ajouter de nouveaux medias</label>
                            <label for="medias" id="upload-dropzone" class="upload-dropzone w-100">
                                <span class="upload-dropzone-icon">
                                    <i class="bi bi-cloud-arrow-up"></i>
                                </span>
                                <h5 class="mb-2">Glisse de nouveaux medias ici</h5>
                            ...
                            <input type="file" name="medias[]" id="medias" class="upload-input" accept=".jpg,.jpeg,.png,.webp,.mp4,.mov,.avi,.webm" multiple>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Previsualisation des nouveaux medias</label>
...
@push('scripts')
    <script>
        const initEditGalleryPage = function () {
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initEditGalleryPage);
        } else {
            initEditGalleryPage();
        }
    </script>

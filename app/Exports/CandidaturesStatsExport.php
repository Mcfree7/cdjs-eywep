...
class CandidaturesStatsResumeSheet implements FromCollection, WithHeadings, WithTitle, WithStyles, WithColumnWidths
{
    ...
    public function collection(): Collection
    {
        ...
        $retenue     = (clone $query)->where('statut', 'retenue')->count();
        $rejetee     = (clone $query)->where('statut', 'rejetee')->count();
        $nbPays      = (clone $query)->distinct('pays')->count('pays');
        $nbProjets   = (clone $query)->distinct('project_id')->count('project_id');
        ...
        $rows = collect([
            ...
            ['',                             ''],
            ['Nombre de pays représentés',   $nbPays],
            ['Nombre de projets concernés',  $nbProjets],
        ]);

        if (!empty($this->filters['project_id'])) {
            $project = \App\Models\Project::find($this->filters['project_id']);
            array_unshift($rows[0], ['Filtre projet', $project?->titre ?? $this->filters['project_id']]);
        }
...
class CandidaturesStatsParPaysSheet implements FromCollection, WithHeadings, WithTitle, WithStyles, WithColumnWidths
{
    ...
    public function headings(): array
    {
        return ['Pays', 'Total', 'En attente', 'Retenue', 'Rejetée', '% Retenue'];
    }
    ...
    public function collection(): Collection
    {
        ...
        $rows = $query->selectRaw('pays, COUNT(*) as total,
    ...
    public function columnWidths(): array
    {
        return ['A' => 25, 'B' => 10, 'C' => 12, 'D' => 10, 'E' => 10, 'F' => 12];
    }
}
...
class CandidaturesStatsParProjetSheet implements FromCollection, WithHeadings, WithTitle, WithStyles, WithColumnWidths
{
    ...
    public function headings(): array
    {
        return ['Projet', 'Total', 'En attente', 'Retenue', 'Rejetée', '% Retenue'];
    }
    ...
    public function collection(): Collection
    {
        ...
        $rows = $query->selectRaw('project_id, COUNT(*) as total,
            SUM(CASE WHEN statut = "en_attente" THEN 1 ELSE 0 END) as en_attente,
            SUM(CASE WHEN statut = "retenue" THEN 1 ELSE 0 END) as retenue,
            SUM(CASE WHEN statut = "rejetee" THEN 1 ELSE 0 END) as rejetee')
            ->groupBy('project_id')
            ...
            ->get()
            ->map(function ($r) {
                $titre = $r->project?->titre ?? 'Projet #' . $r->project_id;
                return [
                    $titre,
                    $r->total,
                    $r->en_attente,
    ...
    public function columnWidths(): array
    {
        return ['A' => 40, 'B' => 10, 'C' => 12, 'D' => 10, 'E' => 10, 'F' => 12];
    }
}
...
class CandidaturesListeSheet implements FromCollection, WithHeadings, WithTitle, WithStyles
{
    ...
    public function headings(): array
    {
        return ['#', 'Nom', 'Prénom', 'Email', 'Téléphone', 'Pays', 'Sexe', 'Projet', 'Statut', 'Date'];
    }
    ...
    public function collection(): Collection
    {
        ...
        return $query->orderBy('created_at', 'desc')->get()->map(fn($c) => [
            $c->id,
            $c->nom,
            $c->prenom,
            $c->email,
